...
def _get_curriculum_stage(env) -> torch.Tensor:
    """Return per-env curriculum stage tensor (0=upright focus, 1=rotation focus)."""
...
# How many interval calls before checking promotion (throttles curriculum updates)
...
def advance_curriculum_stage(env, env_ids: torch.Tensor = None) -> None:
    ...
    Called as EventTerm with mode='interval'; uses function attr as call counter.
    """
    ...
    # Promote to stage 1 when env can keep screwdriver upright and stable
